...
def performance_evaluation(params, iter_train = None, iter_test = None):
	...
	# Copy training sample iterable for sequence synthesis and mixed samples set production.
...
def get_args():
	...
	parser.add_argument('--data_fraction_lower_bound', '-dfl', type=float, default=0.0, help='lower bound for ???')
	parser.add_argument('--data_fraction_upper_bound', '-dfu', type=float, default=1.0, help='????????')
	parser.add_argument('--data_fraction_chunks', '-dfc', type=int, default=10, help='????????')
...
def learning_curve(params):
	...
	if not(check_data_fractions_integrity(data_fraction_lower_bound, data_fraction_upper_bound, data_fraction_chunks)):
		sys.stdout.write('Inconsistent data fraction!')
		sys.exit
	
	data_fractions = list(np.linspace(data_fraction_lower_bound, data_fraction_upper_bound, data_fraction_chunks))
	...
	for i,data_fraction in enumerate(data_fractions):
		...
		mroct, maprt, mrocs, maprs, elapsed_time = batch_performance_evaluation(params, iter_train = iter_train_, iter_test = iter_test_, 
